...
certs_data = [
    ...
    [Paragraph("정보처리기사", normal_style), Paragraph("한국산업인력공단", normal_style), Paragraph("2022.06.17", normal_style), Paragraph("222011******", normal_style)],
    [Paragraph("ADsP(데이터분석준전문가)", normal_style), Paragraph("한국데이터산업진흥원", normal_style), Paragraph("2021.04.09", normal_style), Paragraph("ADsP-028******", normal_style)],
    [Paragraph("SQLD(SQL 개발자)", normal_style), Paragraph("한국데이터산업진흥원", normal_style), Paragraph("2020.06.30", normal_style), Paragraph("SQLD-037****", normal_style)],
    [Paragraph("GTQ 그래픽기술자격 1급", normal_style), Paragraph("한국생산성본부", normal_style), Paragraph("2015.02.13", normal_style), Paragraph("GA01G151********", normal_style)],
    [Paragraph("CS Leaders(관리사)", normal_style), Paragraph("한국정보평가협회", normal_style), Paragraph("2016.10.18", normal_style), Paragraph("160060****", normal_style)],
]
...
# 프로젝트 1: Baseball AI Coach
story.append(Paragraph("<b>1. Baseball AI Coach (야구 AI 코치)</b> | 2026.07.24 ~ 진행 중", project_title_style))
story.append(Paragraph("<i>기능 요약: 실시간 수비 시프트, 경기 로그, 스트라이크 존 히트맵 및 대화형 AI 전술 피드백 KBO 시뮬레이터</i>", normal_style))
story.append(Paragraph("• <b>[프론트/백엔드 아키텍처]</b> Vite + React 19 및 Spring Boot 3.3.2 기반 실시간 KBO 시뮬레이터 전반 설계 및 개발.", bullet_style))
story.append(Paragraph("• <b>[수비 시프트 시각화]</b> 대화형 SVG 야구장 내 9명의 야수 드래그 수비 시프트 기능 및 AI 가이드 오버레이 구현.", bullet_style))
story.append(Paragraph("• <b>[Gemini AI 비동기 피드백]</b> Spring AI ChatModel 기반 Gemini API와 연동, 500ms 디바운스 및 AbortController 레이스 컨디션 차단 설계로 <b>비동기 전술 피드백 화면 정합성 100% 확보</b>.", bullet_style))
story.append(Paragraph("• <b>[히트맵 시각화]</b> KBO 표준 13분할 스트라이크 존 그리드 입력 및 투구 코스 실시간 히트맵(Heatmap) 시각화 위젯 연동.", bullet_style))
story.append(Spacer(1, 4))

# 프로젝트 2: ZIPT
story.append(Paragraph("<b>2. ZIPT (전세사기 방지 플랫폼)</b> | 2026.06.15 ~ 2026.07.10", project_title_style))
...
# 프로젝트 4: CodeMate
story.append(Paragraph("<b>4. CodeMate (개발자 스터디 매칭 API)</b> | 2026.06.05 ~ 2026.06.10", project_title_style))
...
# 5. 학력 및 주요 교육/대외활동 (Education & Experience)
